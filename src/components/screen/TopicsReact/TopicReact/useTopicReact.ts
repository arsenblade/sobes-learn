...
export const useTopicReact = (id: string) => {
  const { isLoading: isLoadingCurrentTopic, data: currentTopic } = useQuery(['current topic', id], () => topicService.getById(id), {
    onError: () => {
      MyToast('Не удалось загрузить тему', false);
    },
  });

  const { isLoading: isLoadingAllTopics, data: allTopics } = useQuery(['all topics'], () => topicService.getAll());
